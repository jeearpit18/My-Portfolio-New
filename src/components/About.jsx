import stackImage from '../assets/about/arpit-avatar.png';
import { aboutContent, personalInfo } from '../data/portfolioData';

const badgeCodes = ['AWS', 'API', 'WEB'];

const FocusBadge = ({ label, code, delay }) => (
  <div
    data-aos="zoom-in"
  ...
  >
    <div className="flex flex-col items-center gap-3">
      <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-black text-white border border-white/20 shadow-[0_12px_35px_rgba(0,0,0,0.25)] flex items-center justify-center text-sm md:text-base font-black tracking-widest">
        {code}
      </div>
      <span className="text-xs font-bold text-white/70 uppercase tracking-wider text-center">
        {label}
      </span>
    </div>
  </div>
          ...
          <div className="flex items-center gap-6 md:gap-10 mt-8 flex-wrap">
            {aboutContent.techStack.map((item, index) => (
              <FocusBadge
                key={item}
                label={item}
                code={badgeCodes[index] || item.slice(0, 3).toUpperCase()}
                delay={String(300 + index * 150)}
              />
            ))}
          </div>
        </div>
